...
from .model import (
    ft_net, ft_net_dense, ft_net_EF4, ft_net_EF5, ft_net_EF6,
    ft_net_IR, ft_net_NAS, ft_net_SE, ft_net_DSE, PCB, CPB,
    ft_net_angle, ft_net_arc)
...
def load_network(name, cfg):
    ...
    config_path = os.path.join(root_dir, cfg.AICITYREID.CONFIG_PATH)
    ...
    if opt.use_dense:
        model = ft_net_dense(
            opt.nclasses, opt.droprate, opt.stride, None, opt.pool)
    elif opt.use_NAS:
    ...
    # load model
    model_path = os.path.join(root_dir, cfg.AICITYREID.MODEL_PATH)
    print('Load AICityReID model from %s' % model_path)
    network = model
    try:
